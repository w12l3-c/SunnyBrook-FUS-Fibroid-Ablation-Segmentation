...
def deeplabv3_run():
    ...
    if len(train_muscle_dataset)%BATCH_SIZE == 1 or len(val_muscle_dataset)%BATCH_SIZE == 1:
        # Batch number of 1 will cause error in batchnorm
        train_dataloader = create_dataloader(train_muscle_dataset, transform, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=True)
        val_dataloader = create_dataloader(val_muscle_dataset, transform, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=True)
    else:
        train_dataloader = create_dataloader(train_muscle_dataset, transform, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=False)
        val_dataloader = create_dataloader(val_muscle_dataset, transform, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=False)    
    ...
    # Save the model
    save_path = f"/mnt/HDD_1TB/Wallace/Code/Seg2D/trained_models/DeepLabV3_Muscle_{datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S')}.pth"
...
def deeplabv3_inference():
    # Prepare dataset
    dataset = test_muscle_dataset
    
    # Load model
    save_path = ""
    model, transform, loss_fn, optimizer, scheduler = DeepLabV3.DeepLabV3(in_channels=3, num_classes=1, size='regular')
    model = load_model_torch(model, save_path)
...
def unet_run(): 
    ...
    if len(train_muscle_dataset)%BATCH_SIZE == 1 or len(val_muscle_dataset)%BATCH_SIZE == 1:
        # Batch number of 1 will cause error in batchnorm
        train_dataloader = create_dataloader(train_muscle_dataset, transform, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=True, collate_fn=None)
        val_dataloader = create_dataloader(val_muscle_dataset, transform, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=True, collate_fn=None)
    else:
        train_dataloader = create_dataloader(train_muscle_dataset, transform, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=False, collate_fn=None)
        val_dataloader = create_dataloader(val_muscle_dataset, transform, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=False, collate_fn=None)    
    ...
    # Save the model
    save_path = f"/mnt/HDD_1TB/Wallace/Code/Seg2D/trained_models/Unet_Muscle_{datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S')}.pth"
    save_model_torch(best, save_path)
    print(f"Model saved at {save_path}")
...
def unet_inference():
    ...
    saved_model = "/mnt/HDD_1TB/Wallace/Code/Seg2D/trained_models/Unet_Muscle_2023-08-01_18:02:56.pth"
    model = Unet.auto_UNET(in_channels=3, num_classes=2)
    model = load_model_torch(model, saved_model)
    model = model.to(device)
    
    # Run Inference function
    Unet.predict_UNET(model, test_muscle_dataset, device)
...
def unetpp_run(): 
    ...
    # Save the model
    save_path = f"/mnt/HDD_1TB/Wallace/Code/Seg2D/trained_models/Unetpp_Muscle_{datetime.datetime.now().strftime('%Y-%m-%d_%H')}.pth"
    save_model_torch(best, save_path)
    print(f"Model saved at {save_path}")
...
def unetpp_inference():
    ...
    # Run Inference function
    Unetpp.predict_UNET(model, test_muscle_dataset, device)
